...
const PUSH_MESSAGES = {
  ...
  rating_reminder: (d) => ({ title: 'Rate Your Session',    body: d.sessionTitle ? `How was "${d.sessionTitle}"? Don't forget to rate your training partner.` : "Don't forget to rate your training partner." }),
  session_nearby:  (d) => ({ title: `New ${d.sport || ''} Session`.trim(), body: d.location ? `${d.postedBy?.name || 'Someone'} posted a session near ${d.location}` : `${d.postedBy?.name || 'Someone'} posted a ${d.sport || 'training'} session` }),
};
