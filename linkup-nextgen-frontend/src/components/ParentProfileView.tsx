...
import { Camera, LogOut, KeyRound, MapPin, User, ChevronRight, Check, X, Eye, EyeOff } from 'lucide-react';
import { parentProfile, avatarThumb, auth as authApi } from '../lib/api';
...
export function ParentProfileView() {
  ...
  const [view, setView] = useState<'main' | 'children' | 'password'>('main');
    ...
    return (
      <div className="h-full flex flex-col">
        <button
          ...
          className="absolute top-14 left-4 z-10 p-1.5 bg-white/90 backdrop-blur rounded-full shadow"
        />
        <ChildProfilesView />
        <button
          onClick={() => setView('main')}
          className="absolute top-[72px] left-4 z-20 p-1.5 bg-white rounded-full shadow flex items-center justify-center"
        >
        ...
        <div className="px-6 py-6 space-y-4">
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-3 pr-11 rounded-xl border-2 border-slate-200 text-slate-900 placeholder-slate-400 focus:border-emerald-400 focus:outline-none transition-colors"
            />
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { user: updated } = await parentProfile.uploadAvatar(token, file);
      updateUser(updated);
  ...
  const saveLocation = async () => {
    ...
    try {
      const { user: updated } = await parentProfile.update(token, { location: locationDraft.trim() });
      updateUser(updated);
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            <div className="w-20 h-20 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-full flex items-center justify-center text-white font-bold text-2xl overflow-hidden border-3 border-white/30">
              {parent?.avatar
                ? <img src={avatarThumb(parent.avatar, 160)!} alt={parent?.name} className="w-full h-full object-cover" />
          ...
          <div className="flex-1 min-w-0">
            <p className="text-white font-bold text-xl truncate">{parent?.name}</p>
            <p className="text-emerald-300 text-sm">{parent?.email}</p>
            <span className="inline-block mt-1.5 bg-white/15 text-white text-xs font-semibold px-2.5 py-0.5 rounded-full">
              Parent
            </span>
          </div>
        </div>
      ...
      <div className="px-6 -mt-4 pb-8 space-y-4">
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-1">
            ...
            {!editingLocation && (
              <button onClick={() => { setLocationDraft(parent?.location || ''); setEditingLocation(true); }}
          ...
          {editingLocation ? (
            <div className="flex gap-2 mt-2">
              <input
                ...
                autoFocus
                className="flex-1 px-3 py-2 rounded-xl border-2 border-emerald-300 text-slate-900 text-sm focus:outline-none focus:border-emerald-500"
              />
              ...
              <button onClick={() => setEditingLocation(false)} className="px-3 py-2 bg-slate-100 text-slate-600 rounded-xl">
                <X className="w-4 h-4" />
              </button>
            </div>
          ) : (
            <p className="text-slate-500 text-sm mt-0.5">{parent?.location || 'Not set'}</p>
          )}
        </div>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-slate-900">My Children</p>
            <p className="text-sm text-slate-500">
              {childCount === 0 ? 'No children added yet' : `${childCount} child${childCount !== 1 ? 'ren' : ''} added`}
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-4 hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0"
          >
