...
export function ExpiredSessionsModal({ onDismiss }: { onDismiss: () => void }) {
  const { token } = useAuth();
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-[300] flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white rounded-t-3xl sm:rounded-2xl w-full sm:max-w-md max-h-[80vh] flex flex-col" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
        {/* Header */}
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex-shrink-0">
          <button onClick={() => onDismiss()} className="w-full text-sm text-slate-500 hover:text-slate-700 transition-colors">
            Remind me later
          </button>
        </div>
      </div>
